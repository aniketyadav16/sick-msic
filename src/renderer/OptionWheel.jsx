...
const OptionWheel = ({
  ...
  const playTick = useCallback(() => {
    ...
    if (!soundUrl) return;
    const resolvedSound = typeof soundUrl === 'string' && soundUrl.startsWith('/') && !soundUrl.startsWith('//') ? '.' + soundUrl : soundUrl;
    const now = performance.now();
    if (now - lastTickRef.current < 70) return;
    lastTickRef.current = now;
    if (!audioRef.current || audioUrlRef.current !== resolvedSound) {
      audioRef.current = new Audio(resolvedSound);
      audioRef.current.preload = 'auto';
      audioUrlRef.current = resolvedSound;
    }
